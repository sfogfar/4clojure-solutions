...
(fn [& funs]
  (fn [& args]
    (let [funs-to-apply (reverse funs)
          first-fun (first funs-to-apply)
          first-res (apply first-fun args)]
      (reduce (fn [res function-to-apply]
                (function-to-apply res))
              first-res (rest funs-to-apply)))))

;; Problem 59, Juxtaposition
(fn [& funs]
  (fn [& args]
    (reduce (fn [res fun-to-apply]
              (conj res (apply fun-to-apply args)))
            [] funs)))

;; Problem 60, Sequence Reductions
(defn red
  ([f s] (conj [(f s)] (red f (rest s)))))
...
(fn iter [fun prev]
  (lazy-seq (cons prev (iter fun (fun prev)))))

;; Problem 63, Group a Sequence
(defn my-group-by [fun sq]
  (reduce (fn [m x] (let [k (fun x)
                          stored-v (get m k [])
                          updated-v (conj stored-v x)]
                      (assoc m k updated-v))) {} sq))
